...
const form = useForm({
  ...
  prefecture: props.userAddress.prefecture,
  ...
  is_default: Boolean(props.userAddress.is_default),
});
...
const update = () => {
  if (Object.keys(form.errors).length === 0) {
    ...
    form.post(route("admin.userAddress.update", props.userAddress.id), {
      onFinish: () => {
      ...
      <div class="flex sm:flex-row items-center justify-between">
        ...
        <Breadcrumb
          :items="[
            ...
            {
              name: 'Address Edit',
              url: route('admin.userAddress.edit', user.id),
            },
          ]"
        />
      </div>
    ...
    <UserLayout>
      <div class="flex justify-between">
        <div class="flex flex-col gap-2">
          <PageSubTitle title="ユーザー住所編集画面" />
          <PageDescription
            description="ユーザーの住所を編集することができます。"
          />
        </div>
