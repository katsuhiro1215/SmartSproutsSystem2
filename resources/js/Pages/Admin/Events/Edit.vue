...
const form = useForm({
  ...
  event_photo_path: props.event.event_photo_path,
  postalcode: props.event.postalcode,
  prefecture: props.event.prefecture,
  city: props.event.city,
  address1: props.event.address1,
  address2: props.event.address2,
  phone_number: props.event.phone_number,
  status: Boolean(props.event.status),
  event_start_date: props.event.event_start_date,
  event_end_date: props.event.event_end_date,
  application_start_date: props.event.application_start_date,
  application_start_time: props.event.application_start_time,
  application_end_date: props.event.application_end_date,
  application_end_time: props.event.application_end_time,
});
...
const update = () => {
  if (Object.keys(form.errors).length === 0) {
    ...
    form.post(route("admin.event.update", event.id), {
      onFinish: () => {
      ...
      <div class="flex sm:flex-row items-center justify-between">
        ...
        <Breadcrumb
          :items="[
            ...
            { name: 'Event', url: route('admin.event.edit', event.id) },
          ]"
        />
      </div>
    </template>
    ...
    <EventLayout>
      <div class="flex justify-between p-5">
        <div class="w-1/2 flex flex-col gap-2">
          <PageSubTitle title="イベント編集画面" />
          <PageDescription
            description="イベントの編集をすることができます。"
          />
        </div>
