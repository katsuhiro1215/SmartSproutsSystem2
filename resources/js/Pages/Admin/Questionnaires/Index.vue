...
const props = defineProps({
  allQuestionnaires: Object,
  questionnaires: Object,
  deletedQuestionnaires: Object,
});

// Lifecycle
onMounted(() => {
  console.log("questionnaires:", props);
});
...
// Tabs
const tabs = [
  { name: "allQuestionnaires", label: "全アンケート一覧" },
  { name: "questionnaires", label: "アンケート一覧" },
  { name: "deletedQuestionnaires", label: "削除済アンケート一覧" },
];
...
// Questionnaires
const currentQuestionnaires = computed(() => {
  return props[activeTab.value];
});

// 復活処理
const restoreQuestionnaire = (id) => {
  form.put(route("admin.questionnaire.restore", id));
};
...
const confirmDeletion = () => {
  if (entityType.value === "delete") {
    form.delete(route("admin.questionnaire.destroy", currentEntity.value.id));
  } else if (entityType.value === "forceDelete") {
    form.delete(route("admin.questionnaire.forceDelete", currentEntity.value.id));
  }
    ...
    <QuestionnaireLayout>
      <div class="flex justify-between">
        <div class="flex flex-col gap-2">
          <PageSubTitle title="アンケート一覧" />
          <PageDescription description="アンケート一覧を表示する画面です。" />
        </div>
        <div class="flex justify-end items-center gap-2">
          <PrimaryButton :href="route('admin.questionnaire.create')" buttonType="indigo"
            ><Plus />アンケート新規作成</PrimaryButton
          >
      ...
      <div class="w-full flex flex-col p-5">
        ...
        <div class="w-full flex flex-col">
          <Card
            v-for="item in currentQuestionnaires.data"
            :key="item.id"
            class="w-full"
          >
            <template #content>
              <div class="flex items-center justify-between gap-4">
                <div class="w-1/6">
                  <Avatar
                    :src="item.questionnaire_photo_path
                    ? `/storage/questionnaires/${item.questionnaire_photo_path}`
                    : '/upload/note.png'
                "
                    size="xxl"
                  />
                ...
                >
                  ...
                  >
                    <dl class="flex gap-2 text-sm">
                      <dt>公開開始日時期間:</dt>
                      <dd>{{ item.start_date }} - {{ item.start_time }}</dd>
                    </dl>
                    <dl class="flex gap-2 text-sm">
                      <dt>公開終了日時:</dt>
                      <dd>{{ item.end_date }} - {{ item.end_time }}</dd>
                    </dl>
                  </div>
                </div>
                ...
                <div class="w-1/6 flex flex-col justify-end items-center gap-4">
                  <PrimaryButton
                    :href="route('admin.questionnaire.show', { questionnaire: item.id})"
                    buttonType="info"
                  >
                    <EyeOutline />
                  </PrimaryButton>
                  <PrimaryButton
                    :href="route('admin.questionnaire.edit', {questionnaire: item.id})"
                    buttonType="warning"
                  >
                    <NoteEdit />
                  </PrimaryButton>
