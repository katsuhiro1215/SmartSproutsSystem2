...
export const validateDescription = (form) => {
    if (!form.description) {
        ...
        form.errors.description = validationMessages.lesson.description;
    } else {
...
export const validateStartDate = (form, establishedDate) => {
    if (form.start_date && !/^\d{4}-\d{2}-\d{2}$/.test(form.start_date)) {
        form.errors.start_date = validationMessages.lesson.start_date;
    } else if (
        form.start_date &&
        new Date(form.start_date) < new Date(establishedDate)
    ) {
        form.errors.start_date = `開始日は店舗の設立日（${establishedDate}）以降の日付を選択してください。`;
    } else {
...
export const validateEndDate = (form) => {
    if (form.end_date && !/^\d{4}-\d{2}-\d{2}$/.test(form.end_date)) {
        form.errors.end_date = validationMessages.lesson.end_date;
    } else if (
        form.start_date &&
        form.end_date &&
        ...
        form.errors.end_date =
            validationMessages.lesson.end_date_after_start_date;
    } else {
