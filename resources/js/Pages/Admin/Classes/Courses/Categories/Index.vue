...
const props = defineProps({
  ...
  deletedCourseCategories: Object,
});

// Lifecycle
onMounted(() => {
  console.log("allCourseCategories:", props);
});
...
// Tabs
const tabs = [
  { name: "allCourseCategories", label: "全コースカテゴリー一覧" },
  { name: "courseCategories", label: "コースカテゴリー一覧" },
  { name: "deletedCourseCategories", label: "削除済コースカテゴリー一覧" },
];
const activeTab = ref("allCourseCategories");
...
// Course Categories
const currentCourseCategories = computed(() => {
  return props[activeTab.value];
});

// 復活処理
const restoreCourseCategory = (id) => {
  form.put(route("admin.courseCategory.restore", id));
};
...
const confirmDeletion = () => {
  if (entityType.value === "delete") {
    form.delete(route("admin.courseCategory.destroy", currentEntity.value.id));
  } else if (entityType.value === "forceDelete") {
    form.delete(route("admin.courseCategory.forceDelete", currentEntity.value.id));
  }
    ...
    <ClassLayout>
      <div class="flex justify-between p-5">
        <div class="flex flex-col gap-2">
          <PageSubTitle title="コースカテゴリー一覧" />
          <PageDescription
            description="コースカテゴリー一覧を表示する画面です。"
          />
        </div>
        <div class="flex justify-end items-center gap-2">
          <PrimaryButton
            :href="route('admin.courseCategory.create')"
            buttonType="indigo"
            ><Plus />コースカテゴリー新規作成</PrimaryButton
          >
              ...
              <div class="p-6 w-full items-center">
                <div class="tab-content pt-0">
                  ...
                  >
                    ...
                    <div v-if="viewMode === 'list'">
                      ...
                      >
                        <template #renderItem="{ item, index }">
                          ...
                          >
                            <Avatar
                              :src="item.course_category_photo_path"
                              size="sm"
                            />
                          ...
                          >
                            <PrimaryButton
                              :href="route('admin.courseCategory.show', item.id)"
                              buttonType="info"
                              class="size-10"
                            >
                              <EyeOutline />
                            ...
                            <PrimaryButton
                              :href="route('admin.courseCategory.edit', item.id)"
                              buttonType="orange"
                              class="size-10"
                            >
                              <NoteEdit />
                            ...
                            <PrimaryButton
                              v-if="activeTab === 'deletedCourseCategories'"
                              @click="restoreCourseCategory(item.id)"
                              buttonType="success"
                              size="10"
                            >
                              <RestoreIcon />
                    ...
                    <!-- グリッド表示 -->
                    <div v-else>
                      <GridView :items="currentCourseCategories.data">
