...
const store = () => {
  if (Object.keys(form.errors).length === 0) {
    ...
    form.post(route("admin.courseCategory.store"), {
      onFinish: () => {
      ...
      <div class="flex sm:flex-row items-center justify-between">
        ...
        <Breadcrumb
          :items="[
            ...
            { name: 'Create', url: route('admin.courseCategory.create') },
          ]"
        />
      </div>
    </template>
     ...
     <ClassLayout>
       <div class="flex justify-between p-5">
         <div class="w-1/2 flex flex-col gap-2">
           <PageSubTitle title="コースカテゴリー新規作成画面" />
           <PageDescription
             description="コースカテゴリーを新規作成する画面です。"
           />
         </div>
